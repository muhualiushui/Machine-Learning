...
test_error=[]
tran_error=[]
test_error.append(1-test(net, testLoader))
tran_error.append(1-test(net, trainLoader)) 
...
print(tran_error)
def graph(c1,c2):
    plt.plot(c1,'ro-',label='test_error')
    plt.plot(c2,'go-',label='train_error')
    plt.legend(loc=2)
    plt.show()
    pass
graph(test_error,tran_error)